...
### Installer file-management model — force-managed vs. preserved vs. merged

`bin/install.js`'s `writer()` treats installed files in three different ways, chosen by whether a project is ever expected to hand-edit that file:

- **Force-managed** (`forceWrite`/`copyManaged`): `skills/`, `agents/` (in every rendered form — `.claude/skills|agents/`, `.agents/skills/`, `.codex/agents/*.toml`, `.drupal-contrib/agents/`), and the Git-guardrail hook scripts (`block-dangerous-git.sh`, `check-command.py`). These are pure toolkit output — nothing here is meant to be customized per project — so every re-run makes the destination match this version of the package exactly: a local edit is silently overwritten, and a skill/agent removed upstream is deleted from the project (only paths this toolkit itself previously wrote, tracked in the lockfile's `files` map, are ever pruned this way — a file a user added by hand under the same directory is never touched). `copyManaged` takes optional `renameFile`/`transformContent` hooks for the Codex `.md` → `.toml` agent conversion, and a `shallow` flag to keep that conversion from recursing into an agent's `references/` subfolder (which may itself contain `.md` files that aren't agents).
- **Merged** (`upsertBlock`): `CLAUDE.md`/`AGENTS.md`. These commonly pre-exist with a project's own content, so instead of writing the whole file, the toolkit's rendered content is wrapped in `<!-- drupal-contrib-claude-skills:begin/end -->` marker comments and upserted — replacing only the marked block (appending it if the file existed before without markers) on every run. Everything outside the block is the project's own and is never touched.
- **Preserved** (`write`/`copy`, the original behavior): everything else — `.claude/settings.json`, `.mcp.json`, `.codex/config.toml`/`hooks.json`/rules, `.drupal-contrib/context.md`. A package update to these applies automatically only if the file on disk still matches what the toolkit itself last wrote (per the lockfile's content hash); a hand-edited file is left alone and the new version is stashed at `.drupal-contrib/proposals/<path>` instead, for the user to merge manually.

If you add a new installed file, decide explicitly which of these three it needs — don't assume `write`/`copy`'s conflict-safe default fits everything.
...
The same script (`templates/hooks/check-command.py`) also blocks reading secret material via common dump commands (`cat`, `grep`, `head`, `cp`, `curl`, etc., via `READ_BINARIES`) and writing to it — via writer commands (`tee`, `dd`, `install`, via `WRITE_BINARIES`) and via shell output redirection (`> .env`, `>> ~/.ssh/id_rsa`), so Codex (which has no per-tool file permission concept — see below) can't be routed around the settings.json deny list by shelling out instead of using its native edit tool. Both directions match the same path set — `.env`/`.env.*` (except common non-secret suffixes like `.example`/`.sample`/`.dist`/`.template`/`.defaults`/`.test`), `*.pem`/`*.key`/`*.pfx`/`*.p12`, `id_rsa*`/`id_ed25519*`, `.netrc`/`.npmrc`, and anything under a `secrets/`, `.ssh/`, or `.aws/` path component. This reuses the hook's existing tokenizer and shell-`-c`-unwrapping, so it isn't bypassed by `bash -c "cat .env"` the way a plain glob match on the raw command string would be — see `READ_BINARIES`/`WRITE_BINARIES`/`SECRET_GLOBS`/`SECRET_DIR_NAMES` in `check-command.py` to add more. `templates/settings.json.template`'s `deny` list adds the Claude-Code-only complement: `Read`/`Edit` denials on the same secret paths (`.env*`, `*.pem`, `*.key`, `*.pfx`, `*.p12`, `id_rsa*`, `id_ed25519*`, `secrets/**`, `~/.ssh/**`, `~/.aws/**`, `~/.netrc`, `~/.npmrc`), which blocks Claude's native file tools even when no Bash command is involved — Codex has no equivalent per-tool permission concept, so for Codex the hook above is the only protection. Never add a `Write(...)` deny rule for these paths: Claude Code's file-permission matcher only checks `Edit(path)` rules for file-write checks, so `Write(...)` is dead weight — the harness itself flags it as an unenforced rule at session start, even when a matching `Edit(...)` rule sits right next to it. `Edit(...)` alone covers every file-editing tool (Write, Edit, NotebookEdit, …). Both layers are best-effort string/glob matching, not a sandbox — see the module docstring in `check-command.py`.
