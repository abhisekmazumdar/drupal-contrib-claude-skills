...
The same script (`templates/hooks/check-command.py`) also blocks reading secret material via common dump commands (`cat`, `grep`, `head`, `cp`, `curl`, etc.) — `.env`/`.env.*` (except common non-secret suffixes like `.example`/`.sample`/`.dist`/`.template`/`.defaults`/`.test`), `*.pem`/`*.key`/`*.pfx`/`*.p12`, `id_rsa*`/`id_ed25519*`, `.netrc`/`.npmrc`, and anything under a `secrets/`, `.ssh/`, or `.aws/` path component. This reuses the hook's existing tokenizer and shell-`-c`-unwrapping, so it isn't bypassed by `bash -c "cat .env"` the way a plain glob match on the raw command string would be — see `READ_BINARIES`/`SECRET_GLOBS`/`SECRET_DIR_NAMES` in `check-command.py` to add more. `templates/settings.json.template`'s `deny` list adds the Claude-Code-only complement: `Read`/`Write`/`Edit` denials on the same secret paths (plus `~/.ssh/**`, `~/.aws/**`, `~/.netrc`, `~/.npmrc`), which blocks Claude's native file tools even when no Bash command is involved — Codex has no equivalent per-tool permission concept, so for Codex the hook above is the only protection. Both layers are best-effort string/glob matching, not a sandbox — see the module docstring in `check-command.py`.
