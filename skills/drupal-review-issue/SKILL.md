...
```bash
# Issue details — try Drupal.org first
drupalorg issue:show <nid> --with-comments --format=llm
# If the response has empty <issue_id> and empty <title>, the project's issue
# queue has migrated to GitLab work items. Fall back to:
...
# Pipeline status — prefer glab for richer/live output
glab ci status -b <branch> -R issue/<project>-<nid>
# fallback:
drupalorg mr:status <nid> <mr-iid> --format=llm

# Top-level MR thread/discussion comments (complements inline threads from fetch.py)
glab mr view <mr-iid> -R issue/<project>-<nid> --comments
```

If the pipeline is failing, stream the failing job's trace:
```bash
glab ci trace -b <branch> -R issue/<project>-<nid>
# fallback:
